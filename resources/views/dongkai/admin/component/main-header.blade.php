                ...
                <ul class="nav navbar-nav">
                    ...
                    <li class="dropdown user user-menu">
                        ...
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                            ...
                            @if(Auth::guard('dongkai_admin')->user()->portrait_img)
                                <img src="{{url(config('common.host.'.env('APP_ENV').'.cdn').'/'.Auth::guard('dongkai_admin')->user()->portrait_img)}}" class="user-image" alt="User Image">
                            @else
                                <img src="/AdminLTE/dist/img/user2-160x160.jpg" class="user-image" alt="User Image">
                            @endif
                            <!-- hidden-xs hides the username on small devices so only the image appears. -->
                            <span class="hidden-xs">{{Auth::guard('dongkai_admin')->user()->nickname}}</span>
                        </a>
                        <ul class="dropdown-menu">
                            <!-- The user image in the menu -->
                            <li class="user-header">
                                @if(Auth::guard('dongkai_admin')->user()->portrait_img)
                                    <img src="{{url(config('common.host.'.env('APP_ENV').'.cdn').'/'.Auth::guard('dongkai_admin')->user()->portrait_img)}}" class="img-circle" alt="User Image">
                                @else
                                    <img src="/AdminLTE/dist/img/user2-160x160.jpg" class="img-circle" alt="User Image">
                                @endif

                                <p>
                                    {{Auth::guard('dongkai_admin')->user()->nickname}} - 管理员
