...
class MenuRepository {
    ...
    public function get_items_list_datatable($post_data)
    {
        ...
        $org_id = Auth::guard('dongkai_admin')->user()->org_id;
//        $query = OrgItem::select("*")->with(['admin','menus'])->where('id',$decode_id);
    ...
    public function view_items()
    {
        ...
        {
            unset($mine->id);
            return view('dongkai.admin.menu.items')->with(['encode_id'=>$id, 'data'=>$mine]);
        }
    ...
    {
        return view('dongkai.admin.menu.edit')->with(['operate'=>'create', 'encode_id'=>encode(0)]);
    }
    ...
    public function view_edit()
    {
        ...
        if($decode_id == 0) return view('dongkai.admin.menu.edit')->with(['operate'=>'create', 'encode_id'=>$id]);
        else
        {
            ...
            {
                unset($mine->id);
                return view('dongkai.admin.menu.edit')->with(['operate'=>'edit', 'encode_id'=>$id, 'data'=>$mine]);
            }
            else return response("该目录不存在！", 404);
        }
    }
...
//            exit($e->getMessage());
    ...
    public function view_sort()
    {
        ...
        return view('dongkai.admin.menu.sort')->with(['data'=>$mine]);
    }
